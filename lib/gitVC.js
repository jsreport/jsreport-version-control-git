...
module.exports = (reporter, options) => {
  ...
  return ({
    async init () {
      ...
      if (reporter.options.extensions.versionControl.remote) {
        reporter.logger.debug('Remote git repository provided')

        reporter.logger.debug('Performing clone and checking out branch: ' + options.branch)
        await utils.repoClone(options.remote, options.dataDirectory, options.branch)
        ...
        await git.Repository.init(options.dataDirectory, 0)

        const repo = await git.Repository.open(options.dataDirectory)
        if ((repo.isEmpty()) === 0) {
          return
        }

        reporter.logger.debug('Version control for git is making initial commit')
        fs.promises.writeFile(path.join(options.dataDirectory, '.gitignore'), 'settings\nstorage/**/*')
