    ...
    async init () {
      ...
      if (remote) {
        ...
        if (fs.existsSync(path.join(dataDirectory, '.git'))) {
          if (await utils.hasRemoteAsOrigin(dataDirectory, remote)) {
            reporter.logger.debug('Matching repository present in data directory, attempting to merge')
            await this.localChangesFromRemote()
            await this.performMerge()
            await this.pushChanges()
          } else {
            reporter.logger.debug('Mismatched repository in data directory, overwriting')
            utils.repoClone(remote, dataDirectory)
          }
        } else {
