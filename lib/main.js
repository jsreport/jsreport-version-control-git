...
module.exports = (reporter, definition) => {
  ...
  const remote = reporter.options.extensions.versionControl.remote.url

  const options = {
    remote: remote
  }

  if (reporter.options.extensions.versionControl.remote.branch) {
    options.branch = reporter.options.extensions.versionControl.remote.branch
  } else {
  ...
  reporter.initializeListeners.add(definition.name, () => {
    if (reporter.express) {
      reporter.express.exposeOptionsToApi(definition.name, {
        remote: reporter.options.extensions.versionControl.remote.url
      })
    }
  })
  ...
  reporter.on('express-configure', (app) => {
    ...
    app.post('/api/version-control-git/push', (req, res, next) => {
      if (!reporter.options.extensions.versionControl.remote.allowPush) {
